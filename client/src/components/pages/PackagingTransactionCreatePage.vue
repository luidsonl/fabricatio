...
const handleSubmit = async (e: Event) => {
  ...
  router.push('/transactions/packaging')
}
</script>
...
<template>
  <MainLayout>
    <div class="p-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-6">
      <h1 class="text-2xl font-bold text-slate-800">New Packaging Transaction</h1>
      <form @submit="handleSubmit" class="max-w-md space-y-4 bg-white p-6 rounded-lg shadow-sm border border-slate-100">
        ...
        <div class="flex gap-2 pt-2">
          <Button type="button" @click="router.push('/transactions/packaging')" class="bg-slate-100 text-slate-700 hover:bg-slate-200">Cancel</Button>
