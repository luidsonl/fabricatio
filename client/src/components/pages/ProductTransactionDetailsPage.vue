...
const transaction = computed(() => store.productTransactions.find((t: any) => t.id === transactionId.value))
</script>
    ...
    <div class="p-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-6">
      ...
      <div v-else class="space-y-6">
        <div class="flex justify-between items-center mb-6">
          ...
          <Button @click="router.push('/transactions/products')" class="bg-slate-100 text-slate-700 hover:bg-slate-200">Back to List</Button>
        </div>

        <Card>
          <div class="space-y-4">
            <div>
              <span class="font-bold block text-sm text-slate-600">Product</span>
              <div class="text-lg font-medium text-slate-900">{{ transaction.productName || `ID: ${transaction.productId}` }}</div>
            </div>
